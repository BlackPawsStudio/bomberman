...
export class StartView {
  ...
  renderUI() {
    ...
    <nav class="nav">
      ...
      }>${model.auth}${model.auth === 'authorized' ? `: ${model.userName}` : ''}</button>
  ...
  async setContinueButtonState() {
    this.uid = model.uid;
    const continueButton = selectorChecker(document, '.continue') as HTMLButtonElement;
